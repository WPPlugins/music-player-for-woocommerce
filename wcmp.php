	...
	class WooCommerceMusicPlayer
	{
		...
		*/
		...
		public function admin_init()
		{
			...
			add_filter("plugin_action_links_".plugin_basename(__FILE__), array(&$this, 'menu_links'));
		} // End admin_init

		public function menu_links($links)
		{
			array_unshift(
			$links,
		...
		public function save_post()
		{
			...
			$global_settings = array( 'preload' => 'metadata' );
			if(isset($_POST['_wcmp_play_all'])) $global_settings['play_all'] = true;
			if(
				isset($_POST['_wcmp_preload']) &&
				in_array($_POST['_wcmp_preload'], array('none', 'metadata', 'auto'))
			) $global_settings['preload'] = $_POST['_wcmp_preload'];
		...
		public function enqueue_resources()
		{
			...
			$play_all = (!empty($global_settings[ 'play_all' ])) ? true : false;
		...
		public function replace_playlist_shortcode($atts)
		{
			...
			if( !empty( $products ) )
			{
				...
				foreach( $products as $product )
				{
					...
					$output .= '<ul class="wcmp-widget-playlist '.esc_attr( $class ).' '.esc_attr( ( $product->ID == $current_post_id ) ? 'wcmp-current-product' : '' ).'">';
					foreach( $audio_files as $index => $file )
					{
						$audio_url = $this->generate_audio_url( $product->ID,  $index );
						$audio_tag = apply_filters('wcmp_widget_audio_tag', $this->get_player( $audio_url, array( 'player_controls' => 'track', 'player_style' => $player_style, 'media_type' => ((gettype($file) == 'object') ? $file->media_type : $file[ 'media_type' ]), 'id' => $index ) ), $product->ID, $index);
		...
		)
		{
			...
			$global_settings = get_option('wcmp_global_settings', array());
			$preload = (!empty($global_settings[ 'preload' ])) ? $global_settings[ 'preload' ] : 'matadata';

			return '<audio '.$id.' preload="'.esc_attr($preload).'" class="wcmp-player '.esc_attr($args[ 'player_controls' ]).' '.esc_attr($args[ 'player_style' ]).'"><source src="'.esc_url($audio_url).'" type="audio/'.esc_attr($args['media_type']).'" /></audio>';
		...
		public function include_main_player()
		{
			...
			if( !empty( $files ) )
			{
				...
				$audio_tag = apply_filters('wcmp_audio_tag', $this->get_player($audio_url, array( 'player_controls' => $player_controls, 'player_style' => $player_style, 'media_type' => ((gettype($file) == 'object') ? $file->media_type : $file[ 'media_type' ] ))), $id, $index);
		...
		{
			$id = $product->get_id();
			$_aux = create_function('$arr', 'if(gettype($arr) == "object") $arr->product='.$id.'; else $arr["product"]='.$id.'; return $arr;');
			$product_type = $product->get_type();

			if( $product_type == 'variation' )
			{
				$_files = $product->get_files();
				$_files = array_map( $_aux, $_files );
				...
				switch( $product_type )
				{
					...
					case 'grouped':
						...
						foreach( $children as $key => $child_id )
							$children[$key] = $product->get_child($child_id);

						usort($children, array(&$this,'_sort_list'));
