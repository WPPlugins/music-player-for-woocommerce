...
// include resources
wp_enqueue_style( 'wcmp-admin-style', plugin_dir_url(__FILE__).'../css/style.admin.css' );
...
$wcmp_global_settings = get_option('wcmp_global_settings',array());
if( empty($wcmp_global_settings[ 'preload' ]) ) $wcmp_global_settings[ 'preload' ] = 'metadata';
?>
...
<table class="widefat" style="border-left:0;border-right:0;border-bottom:0;padding-bottom:0;">
	...
	<tr>
		...
		<td><input type="checkbox" name="_wcmp_enable_player" <?php echo (( $enable_player ) ? 'checked' : '' ); ?> /></td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td>
			<label><input type="radio" name="_wcmp_preload" value="none" <?php if($wcmp_global_settings[ 'preload' ] == 'none') echo 'CHECKED'; ?> /> None</label><br />
			<label><input type="radio" name="_wcmp_preload" value="metadata" <?php if($wcmp_global_settings[ 'preload' ] == 'metadata') echo 'CHECKED'; ?> /> Metadata</label><br />
			<label><input type="radio" name="_wcmp_preload" value="auto" <?php if($wcmp_global_settings[ 'preload' ] == 'auto') echo 'CHECKED'; ?> /> Auto</label><br />
			<em>
			<?php
				_e( 'This attribute has a global scope and affects the players in all website pages', 'woocommerce_music_player' );
			?>
			</em>
		</td>
	</tr>
	<tr>
		...
		<td>
			<input type="checkbox" name="_wcmp_play_all" <?php if(!empty($wcmp_global_settings['play_all'])) echo 'CHECKED'; ?> />
			<em>
			<?php
				_e( 'This attribute has a global scope and affects the players in all website pages', 'woocommerce_music_player' );
			?>
	...
	<tr valign="top">
		<td style="color:#DDDDDD;"><?php _e('Select my own demo files', 'woocommerce_music_player'); ?></td>
		<td><input type="checkbox" disabled /></td>
	</tr>
</table>
<table class="widefat" style="border:0;padding-bottom:20px;">
	<tr valign="top" class="wcmp-demo-files">
		<td style="color:#DDDDDD;">
			<div><?php _e('Demo files', 'woocommerce_music_player'); ?></div>
			<table class="widefat">
				<thead>
					<tr>
